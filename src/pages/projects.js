...
const ProjectComponent = ({ width, alt, imageWebp, imagePNG, githubLink, projectName, leftSide }) => {
    return (
        <>
            {leftSide === true ? (<Grid item xs={projectsWidths(width)[0]} />) : (
                <>{width <= 1300 && (<Grid item xs={projectsWidths(width)[0]} />)}</>
            )}
            <Grid item xs={projectsWidths(width)[1]} sx={{display: "flex", justifyContent: "center"}}>
                <Grid container style={{display: "flex", alignItems: "center"}}>
                    <Grid item xs={12} sx={{position: "relative", top: "0", borderRadius: "5px", transition: "0.2s", "&:hover": {top: "-5px"}}}>
                        <picture>
                            ...
                            <img src={imagePNG} alt={alt} height="auto" width="100%" style={{borderRadius: "5px", boxShadow: "0 0px 1px 0px rgb(0 0 0 / 5%), 0 1px 4px rgb(0 0 0 / 5%), 0 10px 20px rgb(0 0 0 / 5%)"}} />
                        </picture>
                    </Grid>
                    <Grid item xs={8} sx={{marginTop: "1%"}}>
                        <Typography variant="h6" sx={{fontFamily: "Source Sans Pro", "&:hover": {color: "#E60268"}, transition: "0.2s"}}><b>{projectName}</b></Typography>
                    </Grid>
                    <Grid item xs={4} sx={{display: "flex", justifyContent: "flex-end", alignItems: "start", marginTop: "3%"}}>
                        <Button href={githubLink} target="_blank" variant="outlined" startIcon={<GitHub />}>
                            <Typography variant="subtitle1" sx={{fontFamily: "Source Sans Pro"}}>Code</Typography>
                        </Button>
                    </Grid>
                </Grid>
            </Grid>
            {leftSide === true ? (<>{width <= 1300 && (<Grid item xs={projectsWidths(width)[0]} />)}</>) : (<Grid item xs={projectsWidths(width)[0]} />)}
        </>
    )
}
